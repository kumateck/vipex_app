import { getItemAsync, setItemAsync, deleteItemAsync } from '@mobile/lib/secure-store-compat';
...
export async function loadAppearanceMode(): Promise<AppearanceMode> {
  const raw = await getItemAsync(APPEARANCE_KEY);
  if (raw === 'light' || raw === 'dark' || raw === 'system') return raw;
  return 'system';
}

export async function saveAppearanceMode(mode: AppearanceMode): Promise<void> {
  await setItemAsync(APPEARANCE_KEY, mode);
}
