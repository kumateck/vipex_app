...
import { canMarkParcelArrived, canUseTransitReceiveScan } from '@mobile/lib/permissions';
...
export function ReceiveScanScreen() {
  const { theme } = useAppearance();
  const { session, withAuth } = useAuth();
  const permissions = session.user?.permissions ?? [];
  const canView = canUseTransitReceiveScan(permissions);
  ...
  const incomingTotalRequestRef = useRef(0);

  const loadIncomingTotal = useCallback(async () => {
    if (!companyId || !branchId) return;
    const requestId = ++incomingTotalRequestRef.current;
    ...
    try {
      ...
      if (requestId === incomingTotalRequestRef.current) {
        setIncomingTotal(response.meta?.totalRecords ?? response.data.length);
      }
    } catch {
      if (requestId === incomingTotalRequestRef.current) setIncomingTotal(null);
    }
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      setScanBusy(false);
      void loadIncomingTotal();
      return () => {
        incomingTotalRequestRef.current += 1;
      };
    }, [loadIncomingTotal]),
  );
  ...
  const loadIncomingList = useCallback(async () => {
    ...
    try {
      ...
      if (!search.trim() && !searchAllCompany) {
        setIncomingTotal(response.meta?.totalRecords ?? response.data.length);
      }
      void hapticTap();
    } catch (err) {
    ...
    <AppScreen refreshing={searchBusy} onRefresh={() => void loadIncomingList()}>
      ...
      {mode === 'scan' ? (
        <AppCard>
          ...
          <ScannerView onCodeScanned={receiveByCode} processing={scanBusy} />
          {!canMarkArrived ? (
