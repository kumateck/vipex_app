# Frontend Architecture & Refactoring Rules

## Objective

Ensure all agents enforce modular, scalable, and high-performance frontend architecture.

## Rules

### 1. Feature-Based Architecture (MANDATORY)

All features MUST follow:

`src/features/<domain>/<feature>/components/<feature-name>/`

No large page files allowed.

### 2. File Size Rule (STRICT)

- Ideal: `<150` lines
- Hard limit: `300` lines
- If exceeded, split the file before continuing.
...
- Do one thing only.
- Not mix UI, logic, API, and state.

### 4. Page Rule (CRITICAL)

Pages MUST be thin wrappers only.

Example:

```tsx
export function ParcelReconciliationCasesPage() {
  return <ParcelReconciliation />;
}
```
...
- Be in its own file.
- Have isolated logic.
- Be lazy-loadable if heavy.

### 7. Logic Separation Rule

All business logic MUST be in hooks. Components are UI only.

### 8. Export Rule (MANDATORY)

All exports MUST go through `index.ts`.

### 9. Final Page Reference Rule (CRITICAL)

Pages MUST re-export like this:

```tsx
export { ParcelReconciliationCasesPage } from '../components/parcel-reconciliation';
```
...
- Use `useMemo` and `useCallback` properly.
- Prevent unnecessary re-renders.
- Avoid prop drilling.

### 11. Zero Regression Rule

Refactoring MUST NOT:

- Change UI.
- Break behavior.
- Modify business logic unintentionally.
...
## Project-Specific UI Rules (Mandatory)

### Date Inputs Policy

Do **not** use native HTML date inputs anywhere in the application UI:

- `input type="date"` is forbidden.
- `input type="datetime-local"` is forbidden.

Always use shared UI components already present in the codebase:

- Use `DatePicker` from `src/components/ui/date-picker.tsx` for single-date selection.
- Use `DateTimePicker` from `src/components/ui/date-time-picker.tsx` for date+time selection.
- Use `DateRangePicker` from `src/components/ui/date-range-picker.tsx` for range selection.

This is a hard architecture rule for consistency and UX parity. When modifying existing pages, replace native date/datetime inputs with these shared components instead of adding new native inputs.
