...
function filterSelectChildren(node: React.ReactNode, query: string): FilterResult {
  if (!query) {
    return {
      node,
      count: React.Children.toArray(node).filter(Boolean).length,
    };
  }
  ...
  if (Array.isArray(node)) {
    const filtered = node.map((child) => filterSelectChildren(child, query));
    return {
      node: filtered.map((item) => item.node),
  ...
  const element = node as React.ReactElement<{ children?: React.ReactNode; value?: string }>;

  if (element.type === SelectItem) {
    const valueText = String(element.props.value ?? '');
    const labelText = flattenText(element.props.children);
    const searchable = `${valueText} ${labelText}`.toLowerCase();
    const match = searchable.includes(query);
    return { node: match ? element : null, count: match ? 1 : 0 };
  }
  ...
  const filteredChildren = React.Children.map(element.props.children, (child) =>
    filterSelectChildren(child, query),
  );
  const count = (filteredChildren ?? []).reduce((sum, item) => sum + item.count, 0);
  const nextChildren = (filteredChildren ?? []).map((item) => item.node);

  if (count === 0 && (element.type === SelectGroup || element.type === SelectLabel)) {
    return { node: null, count: 0 };
  }

  return {
    node: React.cloneElement(element, { ...element.props, children: nextChildren }),
    count,
  };
}
