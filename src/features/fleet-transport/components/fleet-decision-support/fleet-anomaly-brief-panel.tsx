...
import {
  useGenerateFleetAnomalyBriefMutation,
  useGetLatestFleetAnomalyBriefQuery,
} from '@/features/dashboard/api/fleet-anomaly-brief.api';
...
export function FleetAnomalyBriefPanel({ from, to }: Props) {
  const latest = useGetLatestFleetAnomalyBriefQuery();
  const [generate, generateState] = useGenerateFleetAnomalyBriefMutation();

  const brief = generateState.data ?? latest.data;
  const isBusy = latest.isFetching || generateState.isLoading;

  function handleGenerate() {
    void generate({ from, to, branchId: null });
  }
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2 text-base">
          <Icon name="Sparkles" className="h-4 w-4 text-primary" /> AI Fleet Anomaly Brief
        </CardTitle>
        <CardDescription>
      ...
      <CardContent className="space-y-4 text-sm">
        {generateState.error || (latest.error && !brief) ? (
          <p className="text-muted-foreground">
            The fleet anomaly brief isn&apos;t available right now — the figures above are still
            accurate.
          </p>
        ) : null}
