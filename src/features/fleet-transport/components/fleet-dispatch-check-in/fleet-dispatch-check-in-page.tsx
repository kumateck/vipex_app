...
export function FleetDispatchCheckInPage() {
  ...
  const [checkIn, { isLoading }] = useCreateFleetTripCheckInMutation();
  ...
  const checkInEvents = useMemo(() => events.filter((event) => event.eventType === 0), [events]);
      ...
      <div className="w-full space-y-4 p-4">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0">
            <CardTitle>Check-In Operator</CardTitle>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Recent Check-In Events</CardTitle>
          </CardHeader>
          <CardContent className="space-y-2 text-sm">
            {!tripId ? (
              <p className="text-muted-foreground">Select a trip to see check-in history.</p>
            ) : null}
            {tripId && checkInEvents.length === 0 ? (
              <p className="text-muted-foreground">
                No check-in events recorded for this trip yet.
              </p>
            ) : null}
