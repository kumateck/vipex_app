...
const PARCEL_STATUS_LABELS: Record<number, string> = {
  ...
  [ParcelStatus.PARCEL_GROUPED]: 'Parcel Grouped',
...
function detailRow(label: string, value: string | number | null | undefined) {
  return (
    <div className="grid grid-cols-3 gap-2 text-sm" key={label}>
      ...
      <span className="col-span-2 break-words">{value == null || value === '' ? '-' : String(value)}</span>
    </div>
  );
}
...
export function ParcelSuperSearchPage() {
  ...
  return (
    <div className="w-full p-4 space-y-4">
      ...
      <Dialog open={Boolean(selectedParcelId)} onOpenChange={(open) => (!open ? setSelectedParcelId(null) : null)}>
            ...
            <div className="space-y-6">
              <Card>
                ...
                <CardContent className="space-y-2">
                  ...
                  {detailRow('Status', PARCEL_STATUS_LABELS[parcelDetails.parcel.status] ?? parcelDetails.parcel.status)}
                  {detailRow('Source Branch', branchNameById.get(parcelDetails.parcel.sourceId) ?? parcelDetails.parcel.sourceId)}
                  {detailRow('Destination Branch', branchNameById.get(parcelDetails.parcel.destinationId) ?? parcelDetails.parcel.destinationId)}
                  ...
                  {detailRow('Planned To Be Paid', formatCurrency(parcelDetails.parcel.plannedToBePaidPsw))}
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base">Payment Records ({parcelDetails.payments.length})</CardTitle>
                </CardHeader>
                <CardContent className="space-y-2">
                    ...
                    parcelDetails.payments.map((payment) => (
                      <div key={payment.id} className="rounded-md border p-3 text-sm space-y-1">
                        <div className="flex items-center justify-between gap-2">
                          <span className="font-medium">{formatCurrency(payment.grossAmountPsw)}</span>
                          <Badge variant="outline">Receipt: {payment.receiptNo ?? '-'}</Badge>
                        </div>
                        <p className="text-muted-foreground">Received: {formatDate(payment.receivedAt)}</p>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base">Consignment History ({parcelDetails.consignments.length})</CardTitle>
                </CardHeader>
                <CardContent className="space-y-2">
                  {parcelDetails.consignments.length === 0 ? (
                    ...
                    parcelDetails.consignments.map((consignment) => (
                      <div key={`${consignment.consignmentId}-${consignment.addedAt}`} className="rounded-md border p-3 text-sm">
                        <p className="font-medium">{consignment.code}</p>
                        <p className="text-muted-foreground">Date: {formatDate(consignment.consignmentDate)}</p>
                        <p className="text-muted-foreground">
                          Route: {(branchNameById.get(consignment.sourceId) ?? consignment.sourceId)} to{' '}
                          {(branchNameById.get(consignment.destinationId) ?? consignment.destinationId)}
                        </p>
                        <p className="text-muted-foreground">Added: {formatDate(consignment.addedAt)}</p>
                        <p className="text-muted-foreground">Removed: {formatDate(consignment.removedAt)}</p>
                      </div>
                    ))
                  )}
                </CardContent>
