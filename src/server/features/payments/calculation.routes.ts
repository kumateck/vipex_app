...
import {
  ...
  type PaymentCalculationQuery,
} from './calculation.controller';

type PreviewPaymentSplitsQuery = Omit<PaymentCalculationQuery, 'paymentResponsibility'>;

export const paymentCalculationRoutes = new Elysia({ name: 'payment-calculation' })
  // Calculate payment for specific parameters
  .post(
    '/calculate',
    async ({ body }) => calculatePaymentController(body as PaymentCalculationQuery),
    {
      body: t.Object({
        companyId: UUID,
        sourceBranchId: UUID,
  ...
  .post(
    '/preview-splits',
    async ({ body }) => previewPaymentSplitsController(body as PreviewPaymentSplitsQuery),
    {
      body: t.Object({
        companyId: UUID,
        sourceBranchId: UUID,
