...
function fileToRoutePath(relativeToPages: string): string {
  ...
  // Dynamic segments: React Router uses :paramName
  // [id] -> :id
  // [...slug] -> *
...
function generateCode(
  ...
  // Use relative path from src/routes/generated.tsx so client bundler resolves lazy imports
  ...
  for (const layout of layouts) {
    ...
    routeLines.push(`  {`);
    routeLines.push(`    id: 'layout-${layoutIndex}',`);
    routeLines.push(`    path: '${layout.routePath}',`);
    routeLines.push(`    Component: ${layout.componentName},`);
    routeLines.push(`    errorElement: createElement(RouteError),`);
